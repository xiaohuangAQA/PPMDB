...
def searchMore(sType, sId):
    if sType == 'protein':
        ...
        for i in range(len(sites)):
            ...
            pt = []
            pt.append(id)
            pt.append(protein.sequence[int(id)-1])
            news[int(id)-1] = '<span style="color:red">' + news[int(id)-1] + '</span>'
        ...
        if dis:
            for one in dis:
                dis_list.append([protein.sid, one])
        source = protein.evidence_source.replace(" ", "").split(',')
        source_list = []
        for one in source:
            id = one.strip().split(':')[-1]
            if one.startswith('PubMed'):
                source_list.append('<a href = "https://pubmed.ncbi.nlm.nih.gov/{0}/" target = "_blank" >{1}</a>'.format(id, one))
            if one.startswith('UniProtKB'):
                source_list.append('<a href = "https://www.uniprot.org/uniprotkb/{0}/entry" target = "_blank" >{1}</a>'.format(id, one))
            if one.startswith('PDB'):
                source_list.append('<a href = "https://www.rcsb.org/structure/{0}" target = "_blank" >{1}</a>'.format(id, one))
        protein.evidence_source = ', '.join(source_list)
        ...
        total = len(enzyme.uniport_id)
        return enzyme, total+1
    elif sType == 'inhibitor':
        obj = Inhibitor.objects.filter(name=sId)
        inhibitor = []
        for one in obj:
            one.e_id = one.e_id.split(';')
            one.enzyme = one.enzyme.split(';')
            one.enzyme_type = one.enzyme_type.split(';')
            inhibitor.append(one)
        return inhibitor, len(inhibitor)
